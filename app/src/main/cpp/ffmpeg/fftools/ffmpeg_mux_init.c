...
static MuxStream *mux_stream_alloc(Muxer *mux, enum AVMediaType type) {
    ...
    ms->ost.clas = &output_stream_class;
...
static int ost_add(Muxer *mux, const OptionsContext *o, enum AVMediaType type,
    ...
    switch (type) {
        case AVMEDIA_TYPE_VIDEO:
            ret = new_stream_video(mux, o, ost);
            break;
        case AVMEDIA_TYPE_AUDIO:
            ret = new_stream_audio(mux, o, ost);
            break;
        case AVMEDIA_TYPE_SUBTITLE:
            ret = new_stream_subtitle(mux, o, ost);
            break;
        case AVMEDIA_TYPE_ATTACHMENT:
            ret = new_stream_attachment(mux, o, ost);
            break;
    }
    if (ret < 0)
        return ret;
