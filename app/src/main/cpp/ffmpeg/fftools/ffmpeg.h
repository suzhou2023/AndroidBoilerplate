...
{\
    ...
    av_log(NULL, AV_LOG_WARNING, "Multiple %s options specified for stream %d, only the last option '-%s%s%s "                                    \
    SPECIFIER_OPT_FMT_##type"' will be used.\n",\
...
static inline void pkt_move(void *dst, void *src) {
    av_packet_move_ref((AVPacket *) dst, (AVPacket *) src);
}
