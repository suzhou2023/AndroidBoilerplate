...
void show_help_default(const char *opt, const char *arg) {
    ...
    if (show_avoptions) {
        ...
        show_help_children(avfilter_get_class(),
                           AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_AUDIO_PARAM | AV_OPT_FLAG_FILTERING_PARAM);
        show_help_children(av_bsf_get_class(),
...
int ffmpeg_parse_options(int argc, char **argv) {
    ...
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_parse_options=====1\n");
    /* split the commandline into an internal representation */
    ret = split_commandline(&octx, argc, argv, options, groups, FF_ARRAY_ELEMS(groups));
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_parse_options=====2 ret = %d\n", ret);
    if (ret < 0) {
    ...
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_parse_options=====3 ret = %d\n", ret);
    if (ret < 0) {
    ...
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_parse_options=====4 ret = %d\n", ret);
    if (ret < 0) {
...
const OptionDef options[] = {
        ...
        {"shortest_buf_duration", HAS_ARG | OPT_FLOAT | OPT_EXPERT | OPT_OFFSET | OPT_OUTPUT,
        ...
        {"stream_loop", OPT_INT | HAS_ARG | OPT_EXPERT | OPT_INPUT |
                        OPT_OFFSET, {.off = OFFSET(loop)}, "set number of times input stream shall be looped",
         "loop count"},
        ...
        {"stats_enc_pre_fmt", HAS_ARG | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT | OPT_STRING,
         {.off = OFFSET(enc_stats_pre_fmt)},
         "format of the stats written with -stats_enc_pre"},
        {"stats_enc_post_fmt", HAS_ARG | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT | OPT_STRING,
