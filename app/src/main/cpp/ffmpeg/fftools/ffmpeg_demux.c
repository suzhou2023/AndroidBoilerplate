...
static DemuxStream *demux_stream_alloc(Demuxer *d, AVStream *st) {
    ...
    ds->ist.clas = &input_stream_class;
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st) {
    ...
    switch (par->codec_type) {
        case AVMEDIA_TYPE_VIDEO: MATCH_PER_STREAM_OPT(frame_rates, str, framerate, ic, st);
            if (framerate) {
...
int ifile_open(const OptionsContext *o, const char *filename) {
    ...
    f->ts_offset =
            o->input_ts_offset - (copy_ts ? (start_at_zero && ic->start_time != AV_NOPTS_VALUE ? ic->start_time : 0) : timestamp);
